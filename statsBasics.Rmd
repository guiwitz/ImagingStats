...
![](statsBasics-figure/conf_int_sample_pops.png){width=90%}
...
> - Now let's measure (**sample**) FI in 6 different fields of view (FOV), with 10 cells in each FOV. 
> - The mean FI from these FOVs is usually far from the population mean, 5!
...
For any given random sample, the sample mean almost certainly doesn't equal the true mean of the population due to **sampling error**. 

## Sampling distribution II

Let's sample our biological process multiple times (e.g. by taking multiple FOVs) but at different magnifications such that we have a different number of cells per FOV. 
...
> - 95% of the time, the confidence interval WILL contain the population mean.
</div>
